...
TrainingConfig parse_config(const YAML::Node &yaml_config) {
    ...
    config.use_sgd = training_config["use_sgd"].as<bool>(config.use_sgd);
    config.use_sgd_fused = training_config["use_sgd_fused"].as<bool>(config.use_sgd_fused);
...
int main(int argc, char **argv) {
    ...
    sgd_params.lr = training_config.learning_rate;
    sgd_params.weight_decay = training_config.weight_decay;
    sgd_params.momentum = training_config.momentum;

    auto sgd_fused_params = ttml::optimizers::SGDFusedConfig();
    sgd_fused_params.lr = training_config.learning_rate;
    sgd_fused_params.weight_decay = training_config.weight_decay;
    sgd_fused_params.momentum = training_config.momentum;
    ...
    auto select_optimizer =
        [&model, &adamw_params, &sgd_params, &sgd_fused_params, &training_config, &multihost_config]()
        -> std::unique_ptr<ttml::optimizers::OptimizerBase> {
        ...
        } else if (training_config.use_sgd) {
            return std::make_unique<ttml::optimizers::SGD>(get_model_parameters(model), sgd_params);
        } else if (training_config.use_sgd_fused) {
            return std::make_unique<ttml::optimizers::SGDFused>(get_model_parameters(model), sgd_fused_params);
        } else {
    ...
    for (uint32_t epoch = 0; epoch < num_epochs; ++epoch) {
        for (auto [features, target, masks] : train_dataloader) {
            ...
            if (gradient_accumulator_helper.should_step()) {
                ...
                optimizer->step();
                scheduler->step();  // running on hist or device? modify learning rates
